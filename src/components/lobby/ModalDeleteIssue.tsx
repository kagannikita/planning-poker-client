...
import { deleteIssue } from "src/store/IssuesSlice";
...
const ModalDeleteIssue: FC<ModalDeleteIssueProps> = (props) => {
    ...
    return (
      <Modal
        size="tiny"
        open={props.state.modalIsOpen}
        onClose={closeHandler}
      >
        <Modal.Header>Delete issue?</Modal.Header>
        <Modal.Content>
          <p>Are you really want to delete issue <b>{props.state.name}</b> from game session?</p>
        </Modal.Content>
        <Modal.Actions>
